...
export default function __Form({ form, workOrderForm, items, setItems, onSave }) {
	...
	useEffect(() => {
		const warehouseOptions = userWarehouse.map((item) => {
			if (item.status === 1) {
				return {
	...
	const memoizedFilterParameters = useMemo(
		...
		[form.values.warehouse_id, searchValue]
	);

	const { records, scrollRef, handleScrollToBottom } = useInfiniteTableScroll({
		module,
		fetchUrl: PHARMACY_DATA_ROUTES.API_ROUTES.DISPENSE.STOCK_ITEM+'/'+form.values.warehouse_id,
