import { Box, Flex, Button, Stack, Checkbox, Center, rem } from "@mantine/core";
...
export default function _TableOld({ module, open }) {
	...
	const handleRowSubmit = async (rowId) => {
		...
		const oldWarehouses = (originalRow.warehouses || []).map((w) => w.warehouse_id);
		const newWarehouses = formData.warehouses || [];

		const isChanged =
			JSON.stringify(oldWarehouses.sort()) !== JSON.stringify(newWarehouses.sort());
		...
		<Stack>
			{warehouseDropdown.map((mode) => (
				<Checkbox
					...
					checked={
						submitFormData[item.user_id]?.warehouses?.includes(Number(mode.value)) ||
						false
					}
					onChange={(e) => {
