...
function ProductionForm({ height, production_config, id }) {
	const { t } = useTranslation();
	const dispatch = useDispatch();
	const [setSaveCreateLoading] = useState(false);
	...
	const handleProductionConfirmSubmit = async (values) => {
		const properties = ["is_measurement", "is_warehouse", "issue_by_production_batch", "issue_with_warehouse"];
				...
				<Box pt={"xs"} pl={"xs"}>
						...
						<Grid columns={24} gutter={{ base: 1 }}>
							...
							<Grid.Col span={12}>
								<SelectForm
									...
									searchable={false}
									value={
										consumptionMethodData
											? String(consumptionMethodData)
											: production_config?.consumption_method_id
											? String(production_config?.consumption_method_id)
											: null
									}
						...
						<Grid columns={24} gutter={{ base: 1 }}>
							...
							<Grid.Col span={12}>
								<SelectForm
									...
									searchable={false}
									value={
										productionProcedureData
											? String(productionProcedureData)
											: production_config?.production_procedure_id
											? String(production_config?.production_procedure_id)
											: null
									}
					...
					<Box mt={"xs"}>
						<Grid
							...
							onClick={() =>
								form.setFieldValue("is_measurement", form.values.is_measurement === 1 ? 0 : 1)
							}
						>
							...
							<Grid.Col span={1}>
								<Checkbox
									...
									onChange={(event) =>
										form.setFieldValue("is_measurement", event.currentTarget.checked ? 1 : 0)
									}
									styles={() => ({
										input: {
											borderColor: "red",
					...
					<Box mt={"xs"}>
						<Grid
							gutter={{ base: 1 }}
							style={{ cursor: "pointer" }}
							onClick={() => form.setFieldValue("is_warehouse", form.values.is_warehouse === 1 ? 0 : 1)}
						>
							<Grid.Col span={11} fz={"sm"} pt={"1"}>
								{t("Warehouse")}
							</Grid.Col>
							<Grid.Col span={1}>
								<Checkbox
									...
									onChange={(event) =>
										form.setFieldValue("is_warehouse", event.currentTarget.checked ? 1 : 0)
									}
									styles={() => ({
										input: {
											borderColor: "red",
						...
						>
							...
							<Grid.Col span={1}>
								<Checkbox
									...
									onChange={(event) =>
										form.setFieldValue("issue_with_warehouse", event.currentTarget.checked ? 1 : 0)
									}
									styles={() => ({
										input: {
											borderColor: "red",
