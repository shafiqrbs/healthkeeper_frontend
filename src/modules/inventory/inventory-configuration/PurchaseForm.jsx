...
function PurchaseForm({ vendorGroupDropdownData, height, domainConfig, closeDrawer }) {
			...
			<form onSubmit={form.onSubmit(handlePurchaseFormSubmit)}>
				<Box pt={"xs"}>
					...
					<Box bg="gray.1" px="sm" py="xs" mt="xs">
						<Text fz={14} fw={600}>
							Product purchase nature
						</Text>
					</Box>
					<Box pl="sm">
						<Box>{productNature && <>{renderHeadCheckboxes("purchaseProductNature")}</>}</Box>
					</Box>
