...
export default function DiscountForm({ height, config_discount, id }) {
				...
				<Box pt={"xs"} pl={"xs"}>
					...
					<Box mt={"xs"}>
						<Grid
							...
							style={{ cursor: "pointer" }}
							onClick={() =>
								form.setFieldValue("online_customer", form.values.online_customer === 1 ? 0 : 1)
							}
						>
							<Grid.Col span={11} fz={"sm"} pt={"1"}>
								{t("DiscountWithCustomer")}
							...
							<Grid.Col span={1}>
								<Checkbox
									...
									onChange={(event) =>
										form.setFieldValue("online_customer", event.currentTarget.checked ? 1 : 0)
									}
									styles={() => ({
										input: {
											borderColor: "red",
