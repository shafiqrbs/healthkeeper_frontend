...
function SampleInvoiceItemForm(props) {
	...
	useEffect(() => {
		if (searchValue.length > 0) {
			...
			const productFilterData = localProducts.filter((product) =>
				fieldsToSearch.some(
					(field) => product[field] && String(product[field]).toLowerCase().includes(lowerCaseSearchTerm)
				)
			);
	...
	let brandDropdown =
		brandDropdownData && brandDropdownData.length > 0
	...
	let productTypeDropdown =
		productTypeDropdownData && productTypeDropdownData.length > 0
	...
	let productUnitDropdown =
		productUnitDropdownData && productUnitDropdownData.length > 0
			...
			<Grid columns={24} gutter={{ base: 8 }}>
											...
											<Grid.Col span={4}>
												<InputButtonForm
													...
													disabled={
														...
														selectProductDetails.sub_total.toFixed(2)
													}
												/>
											</Grid.Col>
											<Grid.Col span={3}>
													...
													onClick={() => {
														modals.openConfirmModal({
															...
															onConfirm: () => {
																dispatch(
																	deleteEntityData(
																		"inventory/category-group/" + data.id
																	)
																);
															},
														});
				...
				<Grid.Col span={1}>
					<Box bg={"white"} className={"borderRadiusAll"} pt={"16"}>
						<ShortcutInvoice form={form} FormSubmit={"EntityFormSubmit"} Name={"CompanyName"} />
					</Box>
				</Grid.Col>
			</Grid>
		</Box>
