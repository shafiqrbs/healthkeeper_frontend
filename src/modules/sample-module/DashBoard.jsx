...
import { Button, Group, Tabs, rem, Text, Tooltip, Flex, LoadingOverlay } from "@mantine/core";
import { useTranslation } from "react-i18next";
import { IconCircleCheck, IconColorFilter, IconList, IconX } from "@tabler/icons-react";
...
function DashBoard() {
	...
	const tabCreateNewRightButtons = (
		<Group pos={`absolute`} right={0} gap={0}>
			...
			<>
				<Button size="sm" color={`blue.7`} type="submit">
					<LoadingOverlay
	...
	return (
		...
		>
			<Tabs defaultValue="ThreeGrid" onChange={(value) => setActiveTab(value)}>
				<Tabs.List pos={`relative`}>
					<Tabs.Tab value="ThreeGrid" leftSection={<IconList style={iconStyle} />}>
						{t("ThreeGrid")}
					</Tabs.Tab>

					<Tabs.Tab value="TwoGrid" leftSection={<IconList style={iconStyle} />}>
						{t("TwoGrid")}
					</Tabs.Tab>

					<Tabs.Tab value="FullForm" leftSection={<IconList style={iconStyle} />}>
						{t("FullForm")}
					</Tabs.Tab>

					<Tabs.Tab value="FullTable" leftSection={<IconList style={iconStyle} />}>
						{t("FullTable")}
					</Tabs.Tab>

					<Tabs.Tab value="FormAndFile" leftSection={<IconList style={iconStyle} />}>
						{t("FormAndFile")}
					</Tabs.Tab>
