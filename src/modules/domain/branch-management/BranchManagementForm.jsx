...
export default function BranchManagementForm() {
	...
	const maxSwitchesInBox = Math.max(...domainsData.map((domain) => Math.max(domain?.prices?.length || 0, 5)));
			...
			<Grid.Col span={23}>
				...
				>
					<LoadingOverlay
						visible={fetching}
						zIndex={1000}
						overlayProps={{ radius: "sm", blur: 0.5, color: "grape" }}
					/>
							...
							<React.Fragment key={`branch-container-${branch.id}`}>
								...
								<Grid.Col span={7}>
									<Box bg="var(--mantine-color-white)" className="borderRadiusAll">
										<Grid justify="center" align="center" columns={12} gutter={{ base: 8 }}>
											<Grid.Col span={5}>
												<Flex
								...
								<Grid.Col span={8} key={`branch-${index}-prices`}>
									<Box bg="var(--mantine-color-white)" className="">
								...
								<Grid.Col span={8} key={`branch-${index}-settings`}>
									<Box bg="var(--mantine-color-white)" p="" className="">
										<Box className="borderRadiusAll">
											<ScrollArea
												pb={"xs"}
												pt={"xs"}
														...
														{branch?.categories?.map((category, categoryIndex) => (
															<Tooltip key={categoryIndex} mt="8" label={category.name}>
																<Checkbox
																	...
																	style={{
																		paddingLeft:
																			categoryIndex % 3 === 0 ? "16px" : "0px", // Apply left padding for the first column
