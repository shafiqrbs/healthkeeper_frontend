...
const useProConfigDropdownData = () => {
	...
	const proConfigDropdownData = useSelector(
		(state) => state.productionUtilitySlice.proConfigDropdownData
	);

	useEffect(() => {
		if (proConfigDropdownData && Object.keys(proConfigDropdownData).length > 0) {
			if (
				proConfigDropdownData.ProductionProcedure &&
				proConfigDropdownData.ProductionProcedure.length > 0
			) {
				const productionProcedureData = proConfigDropdownData.ProductionProcedure.map(
					(type) => {
			...
			if (
				proConfigDropdownData.ConsumptionMethod &&
				proConfigDropdownData.ConsumptionMethod.length > 0
			) {
				const consumptionMethodData = proConfigDropdownData.ConsumptionMethod.map(
					(type) => {
						return { label: type.name, value: String(type.id) };
					}
				);
...
export default useProConfigDropdownData;
